...
detGenerate (DetGrammar st ps) n = iterate (concatMap f) [st] !! n
...
gen f (Grammar s rules) seed =
    ...
    in
        if checkProbs newRules
        then generate f newRules (s, rands)
        else (error "gen: stochastic rule-set is malformed")

toStoRules :: (Ord a, Eq a) => RuleSet a -> RuleSet a
toStoRules (Sto rs) = Sto rs
toStoRules (Uni rs) = Sto (concatMap insertProb rs')
    where rs' = groupBy (\r1 r2 -> lhs r1 == lhs r2) (sort rs) -- use sameLhs'

insertProb :: [a] -> [(a, Prob)]
insertProb rules = zip rules (repeat probability)
    where probability = 1.0 / fromIntegral (length rules)
...
checkSum rules = abs (1.0 - mySum) <= epsilon
...
replaceFunc rules (s, rands) =
    case s of
        N x -> (getNewRhs rules (N x) (head rands), tail rands)
        a :+ b ->
            let (a', rands') = replaceFunc rules (a, rands)
