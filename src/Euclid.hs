...
superEuclid ks n =
    let
        scheme = euclid (length ks) n -- Divides cycle into (length ks) parts.
    in
        zip ks scheme >>= \(k, subN) -> euclid k subN
...
monoizeDistancesR (r@(o, ds):rs) =
    let
        ...
        goodPositions = uncurry absPositions r >>= \dPos ->
            if elem dPos forbiddenPositions
                then []
                else [dPos]
    in
        if length goodPositions == 0
            then monoizeDistancesR rs
                ...
                in
                    monoRhythm : monoizeDistancesR rs
