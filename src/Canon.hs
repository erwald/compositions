...
findCanons numVoices m = filter (uncurry (check m)) combinations
  where
    ...
    check m i d = null $ findIllegalNotes $ canonize numVoices 1 i d m

findCanonErrors :: Int -> Music Pitch -> [(AbsPitch, Dur, ((Int, Pitch), (Int, Pitch)))]
findCanonErrors numVoices m = mapMaybe (uncurry (findErrors m)) combinations
  where
    combinations = [ (i, d) | i <- canonIntervals, d <- canonOnsetDistances ]
...
findIllegalNotes ms = concatMap findIllegalNotesInGroup $ groupedNotes ms
  where
    accumulator
...
checkNotes (i1, p1) (i2, p2) =
    ...
    in
        case motion i1 i2 of
            Oblique  -> True
            Contrary -> pInterval `elem` [3, 4, 5, 6, 7, 8, 9]
            Parallel -> pInterval `elem` [3, 4, 8, 9]
            Similar  -> pInterval `elem` [3, 4, 5, 8, 9]
