...
canonIntervals :: [Int]
canonIntervals =
    let is = [1, 2, 3, 4, 5, 7, 8, 9, 10, 11, 12]
    in map (* (-1)) (reverse is) ++ [0] ++ is
...
findIllegalNotes ms = concat $ map findIllegalNotesInGroup $ groupedNotes ms
    where
        accumulator :: [(Dur, AbsPitch, Dur, Maybe AbsPitch)] -> (Dur, Maybe AbsPitch) -> [(Dur, AbsPitch, Dur, Maybe AbsPitch)]
        accumulator ns (dur, p) =
            case ns of
                [] -> [(0, 0, dur, p)]
                prevNs@((prevPos, _, prevDur, prevP):_) ->
                    let
                        ...
                        newPitch = p <|> prevP
                    in
                        ((newPos, interval, dur, newPitch):prevNs)

        shortestDur = minimum $ map dur ms
        rationalMod x y
            | x < y = x
            | otherwise = rationalMod (x - y) y
        prepareNotes ns = map (\(pos, i, _, p) -> (pos `rationalMod` shortestDur, (i, p))) $ removeRests $ foldl accumulator [] ns
...
findIllegalNotesInGroup (_, ns) =
    let pairs = [ (x, y) | (x:ys) <- tails ns, y <- ys ]
    in filter (not . (uncurry checkNotes)) pairs
